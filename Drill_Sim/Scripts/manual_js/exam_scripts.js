...
$(function () {
    ...
    // instructor zaprashivaet dannie s klienta
    socket.client.responseInstructor = function () {
        if (role == "driller") {
            socket.server.sendDataToInstructor(roomName, Graphs.DPP, Graphs.SICP, Graphs.PitVa, Graphs.ChPos, Graphs.FP, Graphs.spm, Graphs.BHPd, Graphs.CSHP);
        }
    }
    socket.client.triggerPump = function (id, action) {
        breakOrRestorPump(id, action);
        alert("activating pump difficulty at pumpID: " + id);
    }
...
/*
Track when Instructor connects to current session, share graphs.arrays with him
*/
...
function initialize() {
    ...
    socket.server.getSnapshot(snp_id).done(function (snp) {
        ...
        if (role == "driller") { // driller sharing data
